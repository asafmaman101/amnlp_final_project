...
I trained each sample with a PVP corresponding to the task it belongs to and evaluated all models on each task separatly.
...
As an extention for the multi-tasking training, I used PET to train multi-task models with a few examples.
For each task, I used 1000 examples to train an ensemble of models and generate a soft-labeled dataset.
Finally, I trained one classifier on a 20K size soft-labeled data.
...
\subsection{Single-task Training}
First, when training and evaluating on the same task the performance succeed to match the results with classification head showed in \citet{schick2020exploiting}.
...
Not all improvements point on generalization, for example even though AG's News and Yahoo! has different distributions and classes, there is a strong artifact in comparing them~---~they share two common classes: Sports and Business.
Therefore, we cannot conclude the model learned to generalize to unseen tasks.
This result can be seen more as domain adaptation than a task generalization.

\subtitle{MNLI on QQP}
To overcome this issue, I added paraphrase detection to the set of tasks and evaluate it using QQP dataset.
...
Although these two tasks share some semantic characteristics, they don't share common labels and the task itself is different.
...
meaning that testing MNLI model on QQP got only 8\% less then the model that was fine-tuned on this task.
Although this is still far from concluding that the model infered the task from the description,
such results when training on different task and without seeing examples from the target distribution is significant in my opinion.

\subtitle{PVP Variance}
For some tasks, training and evaluating using different PVPs performed very differently.
For example how QQP performed on Yelp Polarity task, when looking on the extended results (can be found in Table~\ref{tab:single-on-train-set}), we can see that two of the PVPs scored roughly 88\% but one PVP got ~49\%.
This emphesises the importance of PVP choice for performance and generalization.

\subtitle{Supervised Multi-tasking}
The first question raised in this project was how training on all tasks together will affect performance on each task.
The results show that multi-tasking doesn't hurt much~---~the model achieves slightly the same results on each task as if it was fine-tuned to this task specificlly.
...
Excluding "bad" PVPs, by using validation set for instance, can prevent these cases.
...
This means that training on the other tasks benefits with this task.
However, when comparing this score with the single-task traing we see almost the same result (81.7\%) as a model that was trained only on Yahoo!.
...
On these two other tasks, we notice that the scores of the models trained on all other tasks are worse than the baseline model.
This probably stems from the fact that the training tasks contain tasks that are too semantically different and the model struggles to learn to be agnostic to the task.

\subtitle{QQP}
Similarly to single-task training, testing multi-task performance on QQP showed significant improvement over the baseline.
Considering the model wasn't asked to detect paraphrases during training and that it haven't seen paraphrases before evalutaion, it is reasonable to claim that the model succeeded to generalize to this task.\\

To some extent, training a model for multi-tasks benefits with performance on unseen tasks.
However, it still seem that the ability of the model to infer the task from PVP is limited to semantically-close tasks.
The models in the experiments didn't succeeded to generalize to any task, but I think that this limitation is somewhat expected.

\input{multi-use-logits-summary}
\subsection{Few-shot Multi-task Training}
Due to limitations of rescorces, in this setup I limited experiments to four tasks.
Like in the previous setups, the results here are also mixed.
One can observe similar relationships between the tasks.

When training on AG's News or Yahoo, the model succeeds to generalize at some level to the second task.
...
the low performance does not seem to stem from the small amount of example.
Moreover, in comparison to supervised results, PET performs only slightly worse.
This result is not suprising and mostly follows the results revealed in \citet{schick2020exploiting}, but testing it here shows that it can also get the benfits of multi-tasking using the method suggested in this project.
